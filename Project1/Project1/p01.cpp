...
// 1.랜덤하게 사각형을 20~40개 생성
// 2.화면을 클릭하면 생성된 사각형 두배 크기의 지우개 사각형 생성
// 3.지우개 사각형은 검은색이다
// 4.지우개를 클릭한 채로 드래그 하다가 지우개에 닿은 사각형은 삭제, 지우개 크기는 커짐, 색상은 부딪힌 사각형의 색으로 바뀜.
// 5.마우스 버튼을 때면 지우개 사각형은 사라진다.
// 6.다시 클릭하면 지우개 사각형 생성
// 7.'r' 또는 'R' 키를 누르면 모든 사각형이 삭제되고 다시 생성
...
// 1.크기와 색상이 다른 사각형 5~10개 생성
// 2.사각형을 클릭하면 4등분 되어 이동하게 된다.
// 3.등분된 사각형은 점점 작아지고 결국 사라진다.
// 4.제시된 4개의 이동 애니메이션을 모두 보여주면 된다.
#endif
